...
- `python3 scripts/run_a1_a10_demo.py` composes A1-A10 into one local JSON evidence envelope. The S06 runbook at `docs/10_A1_A10_DEMO.md` explains how to inspect `runtime_capability_posture`, `gap_ledger`, per-phase selected surfaces, cache-overlay diagnostics, fallback reasons, timestamps, and fixture proof boundaries without promoting live runtime support.
...
- The A1-A10 baseline runbook (`docs/10_A1_A10_DEMO.md`) and `python3 scripts/run_a1_a10_demo.py` remain fixture-first evidence; their `runtime_capability_posture` and `gap_ledger` must stay unvalidated until separate live runtime smoke proof exists.
...
- **S06**: Make runtime smoke tests the closure gate: capture version/build, plugin load, each requested registration surface, dashboard data-provider hydration, action invocation, native artifacts, approval/request create/read, fallback-rate observability, and import/export/AGENTS.md compatibility before any capability becomes `confirmed`. Use `docs/10_A1_A10_DEMO.md` and `python3 scripts/run_a1_a10_demo.py` to reproduce the fixture baseline and inspect `runtime_capability_posture` plus `gap_ledger` before changing runtime claims.
