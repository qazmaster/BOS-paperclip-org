 ...
 *
 * M016-txa3vu / S09 / T02 — Bounded canonical-reference loader.
 *
 * This module is the single I/O layer for the M016/S09 human review. It
 * reads ONLY the 11 allowlisted runtime-evidence sources registered in
 * `m016-s09-human-review-contract.js::SOURCE_ALLOWLIST`, computes SHA-256
 ...
 * feeds into `contract.buildReviewModel`.
 *
 ...
 *   6. Allowlist parity — only the 11 `source_ref`s in SOURCE_ALLOWLIST
 ...
 *
 * The loader does NOT render markdown, does NOT build the review model,
...
// Never throws; encodes all failures into the result row.
// ---------------------------------------------------------------------------
...
function loadCanonicalReferences(input) {
  ...
  const allHashes = {};
  const status = { read: 0, missing: 0, malformed: 0 };
  let byteTotal = 0;
