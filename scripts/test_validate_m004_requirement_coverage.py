...
class ValidateM004RequirementCoverageTests(unittest.TestCase):
    ...
    def test_final_docs_allow_project_managed_gsd_symlink(self) -> None:
        with FixtureRoot() as root:
            project_store = root.parent / "project-gsd-store"
            try:
                project_store.mkdir()
                summary_path = project_store / "milestones" / "M004-osbua3" / "M004-osbua3-SUMMARY.md"
                s05_path = project_store / "milestones" / "M004-osbua3" / "slices" / "S05" / "S05-SUMMARY.md"
                summary_path.parent.mkdir(parents=True, exist_ok=True)
                s05_path.parent.mkdir(parents=True, exist_ok=True)
                final_text = "R012 R013 R014 R015 R016 are validated and covered by GSD summary evidence.\n"
                summary_path.write_text(final_text, encoding="utf-8")
                s05_path.write_text(final_text, encoding="utf-8")
                (root / ".gsd").symlink_to(project_store)

                payload = valid_ledger()
                payload["inputs"]["m004_summary_path"] = ".gsd/milestones/M004-osbua3/M004-osbua3-SUMMARY.md"
                payload["inputs"]["s05_summary_path"] = ".gsd/milestones/M004-osbua3/slices/S05/S05-SUMMARY.md"
                errors, classification = validator.validate(write_ledger(root, payload), root=root, phase="final")
                self.assertEqual([], errors)
                self.assertEqual("final_ready", classification)
            finally:
