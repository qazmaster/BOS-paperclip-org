#!/usr/bin/env python3
"""Run bounded M006 S01 plugin live registration probes and write evidence.

Extends the S00 probe pattern with broader plugin route discovery targeting
routes that S00 did not cover: plugin install by payload, company-scoped install,
plugin upsert, status/health readbacks, settings/plugins discovery, alternate
path segments, and admin routes if reachable.

The runner is intentionally standard-library-only. It reads PAPERCLIP_API_KEY and
PAPERCLIP_BASE_URL from environment, performs sequential probes bounded by
max 32 routes and 15s timeout per request, and writes one canonical M006-S01
evidence artifact. It never asks for secrets, never writes secret values, never
imports Paperclip internals, and never mutates databases or Paperclip source.
When any probe surface is unavailable, it still writes a valid fail-closed
artifact with precise blocker codes.
"""
...
SCHEMA_VERSION = "m006-s01-plugin-live-registration/v1"
PASSING_ARTIFACT_TYPE = "live-evidence"
BLOCKER_ARTIFACT_TYPE = "fail-closed-blocker"
DEFAULT_OUTPUT = Path("runtime-evidence/M006-S01-plugin-live-registration.json")
...
def _make_probe_result(
    *,
    ...
    return {
        ...
        "response_summary": _deep_redact(summary),
    }
...
def probe_extended_route_discovery(
    client: HttpClient,
    company_id: str,
    plugin_key: str,
) -> dict[str, Any]:
    """Probe routes not covered by S00: install, upsert, status, settings, admin."""
    routes: list[tuple[str, str, str, Mapping[str, Any] | None]] = [
        ("p01-r01", "GET", f"/api/plugins/{_quote(plugin_key)}/status", None),
        ("p01-r02", "GET", f"/api/plugins/{_quote(plugin_key)}/health", None),
        ("p01-r03", "GET", f"/api/companies/{_quote(company_id)}/settings/plugins", None),
        ("p01-r04", "GET", f"/api/company/{_quote(company_id)}/plugins", None),
        ("p01-r05", "GET", f"/api/admin/plugins", None),
        ("p01-r06", "GET", f"/api/admin/plugins/{_quote(plugin_key)}", None),
        ("p01-r07", "POST", f"/api/admin/plugins/{_quote(plugin_key)}/reload", None),
        ("p01-r08", "POST", f"/api/admin/plugins/{_quote(plugin_key)}/enable", None),
        ("p01-r09", "POST", f"/api/admin/plugins/{_quote(plugin_key)}/disable", None),
    ]

    results: list[dict[str, Any]] = []
    observed_plugin_keys: list[str] = []
    for probe_id, method, path, body in routes:
        ...
        results.append(_make_probe_result(
            ...
            path=path,
            result=result,
            description="Extended plugin route discovery",
        ))
        keys = _extract_keys(result.get("json"), ("plugins", "registered_plugins", "registeredPluginKeys", "pluginKeys", "plugin_keys"))
        observed_plugin_keys.extend(keys)

    observed_plugin_keys = list(dict.fromkeys(observed_plugin_keys))
    plugin_found = plugin_key in observed_plugin_keys or any(
        r.get("ok") and plugin_key in str(r.get("response_summary", {}).get("json"))
        for r in results
    )

    return {
        "probe": "extended_route_discovery",
        "plugin_key": plugin_key,
        "plugin_found": plugin_found,
        "observed_keys": observed_plugin_keys,
        "results": results,
    }


def probe_plugin_install_attempt(
    client: HttpClient,
    company_id: str,
    plugin_key: str,
    manifest: Mapping[str, Any],
) -> dict[str, Any]:
    """Attempt safe plugin install payloads."""
    display_name = str(manifest.get("display_name") or manifest.get("displayName") or plugin_key)
    safe_install_payload = {
        "plugin_key": plugin_key,
        "display_name": display_name,
        "source": "manifest",
    }
    company_install_payload = {
        "plugin_key": plugin_key,
        "display_name": display_name,
    ...
    routes: list[tuple[str, str, str, Mapping[str, Any] | None]] = [
        ("p02-r01", "POST", "/api/plugins", safe_install_payload),
        ("p02-r02", "POST", f"/api/companies/{_quote(company_id)}/plugins", company_install_payload),
        ("p02-r03", "PUT", f"/api/plugins/{_quote(plugin_key)}", upsert_payload),
        ("p02-r04", "POST", f"/api/admin/plugins", safe_install_payload),
    ]
    ...
    for probe_id, method, path, body in routes:
        ...
        results.append(_make_probe_result(
            ...
            path=path,
            result=result,
            description="Plugin install attempt with safe payload",
        ))
        install_attempted = True
        if result.get("ok"):
            install_succeeded = True
        keys = _extract_keys(result.get("json"), ("plugins", "registered_plugins", "registeredPluginKeys", "pluginKeys", "plugin_key", "pluginKey"))
        observed_plugin_keys.extend(keys)

    observed_plugin_keys = list(dict.fromkeys(observed_plugin_keys))
    return {
        "probe": "plugin_install_attempt",
        "plugin_key": plugin_key,
...
def probe_tool_registry_extended(
    client: HttpClient,
    company_id: str,
    plugin_key: str,
) -> dict[str, Any]:
    """Extended tool registry readback for piko:* tools."""
    paths = [
        f"/api/companies/{_quote(company_id)}/plugins/{_quote(plugin_key)}/tools",
        f"/api/plugins/{_quote(plugin_key)}/tools",
        f"/api/companies/{_quote(company_id)}/tools",
        f"/api/tools",
    ]
    results: list[dict[str, Any]] = []
    ...
    for i, path in enumerate(paths):
        ...
        results.append(_make_probe_result(
            ...
            path=path,
            result=result,
            description="Extended tool registry readback for piko:* tools",
        ))
        keys = _extract_keys(result.get("json"), ("tools", "registered_tools", "registeredToolKeys", "toolKeys", "tool_keys"))
...
def probe_plugin_health_deep(
    client: HttpClient,
    company_id: str,
    plugin_key: str,
) -> dict[str, Any]:
    """Deep plugin health probes including worker status and runtime info."""
    paths = [
        (f"/api/plugins/{_quote(plugin_key)}/health", "Plugin health"),
        (f"/api/plugins/{_quote(plugin_key)}/status", "Plugin status"),
        (f"/api/companies/{_quote(company_id)}/plugins/{_quote(plugin_key)}/health", "Company-scoped plugin health"),
        (f"/api/companies/{_quote(company_id)}/plugins/{_quote(plugin_key)}/status", "Company-scoped plugin status"),
    ]
    results: list[dict[str, Any]] = []
    runtime_info: dict[str, Any] = {"version": None, "build": None, "plugin_loaded": None}
        ...
        if result.get("ok"):
            ...
            if isinstance(payload, Mapping):
                ...
                if isinstance(data, Mapping):
                    version = data.get("version") or data.get("runtime_version") or data.get("plugin_version")
                    build = data.get("build") or data.get("buildId") or data.get("commit")
                    loaded = data.get("loaded") or data.get("plugin_loaded") or data.get("active")
                    if version:
                        runtime_info["version"] = str(version)
                    if build:
                        runtime_info["build"] = str(build)
                    if loaded is not None:
                        runtime_info["plugin_loaded"] = bool(loaded)

    return {
        "probe": "plugin_health_deep",
        "plugin_key": plugin_key,
...
def probe_artifact_regression_smoke(
    ...
    body = {
        "company_id": company_id,
        "title": f"M006 S01 regression smoke {run_label}",
        "body": "Bounded M006 S01 plugin live registration regression smoke issue.",
        "labels": ["bos-light", "m006-s01", "regression-smoke"],
    ...
    doc_key = "m006-s01-evidence"
    doc_body = {
        "title": f"M006 S01 Evidence {run_label}",
        "format": "markdown",
        "body": f"# M006 S01 Plugin Live Registration\n\nRun label: {run_label}\n\nThis document verifies native document creation remains available.",
    }
    ...
    comment_body = {"body": f"M006 S01 regression smoke comment. Run label: {run_label}."}
...
def _compute_blocker_codes(evidence: Mapping[str, Any]) -> list[str]:
    ...
    # P01: extended route discovery
    p01 = _as_mapping(probes.get("extended_route_discovery"))
    ...
    if p01_results:
        ...
        if not first.get("ok"):
            ...
            if status in (401, 403):
                codes.append("extended_route_discovery_auth_denied")
            elif status == 404:
                codes.append("extended_route_discovery_unsupported")
            elif first.get("error") == "timeout":
                codes.append("extended_route_discovery_timeout")
            else:
                codes.append("extended_route_discovery_unavailable")
    if not p01.get("plugin_found"):
        codes.append("plugin_not_found_extended_discovery")

    # P02: plugin install attempt
    p02 = _as_mapping(probes.get("plugin_install_attempt"))
    p02_results = _as_sequence(p02.get("results"))
    if p02_results:
        all_blocked = all(not _as_mapping(r).get("ok") for r in p02_results)
        if all_blocked:
            codes.append("plugin_install_all_blocked")
            ...
            if status in (401, 403):
                ...
                codes.append("plugin_install_invalid_payload")
    if not p02.get("install_succeeded"):
        codes.append("plugin_install_failed")

    # P03: tool registry extended
    p03 = _as_mapping(probes.get("tool_registry_extended"))
    ...
    if p03_results:
        ...
        if not first.get("ok"):
            status = first.get("status_code")
            if status in (401, 403):
                codes.append("tool_registry_extended_auth_denied")
            elif status == 404:
                codes.append("tool_registry_extended_unsupported")
            else:
                codes.append("tool_registry_extended_unavailable")
    if not _as_sequence(p03.get("piko_tools_observed")):
        codes.append("piko_tools_not_observed_extended")

    # P04: plugin health deep
    p04 = _as_mapping(probes.get("plugin_health_deep"))
    p04_results = _as_sequence(p04.get("results"))
    runtime_info = _as_mapping(p04.get("runtime_info"))
    if p04_results:
        first = _as_mapping(p04_results[0])
        if not first.get("ok"):
            status = first.get("status_code")
            if status in (401, 403):
                codes.append("plugin_health_auth_denied")
            elif status == 404:
                codes.append("plugin_health_endpoint_unsupported")
            else:
                codes.append("plugin_health_unavailable")
    if runtime_info.get("plugin_loaded") is not True:
        codes.append("plugin_not_loaded")
...
def _is_passing(evidence: Mapping[str, Any]) -> bool:
    ...
    plugin_found = _as_mapping(probes.get("extended_route_discovery")).get("plugin_found") is True
    install_succeeded = _as_mapping(probes.get("plugin_install_attempt")).get("install_succeeded") is True
    has_piko_tools = bool(_as_sequence(_as_mapping(probes.get("tool_registry_extended")).get("piko_tools_observed")))
    plugin_loaded = _as_mapping(probes.get("plugin_health_deep")).get("runtime_info", {}).get("plugin_loaded") is True
...
def _load_manifest(plugin_key: str) -> Mapping[str, Any]:
    manifest_path = ROOT / "plugin-bos-light" / "manifest.paperclip-plugin.json"
    if manifest_path.exists():
        try:
            ...
            pass
    return {"plugin_key": plugin_key, "display_name": plugin_key}
...
def build_evidence(
    *,
    ...
    run_label = run_label or f"m006-s01-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    evidence: dict[str, Any] = {
        ...
        "runner": {
            "script": "scripts/run_m006_s01_plugin_live_registration.py",
    ...
    if not live_probe_enabled:
        ...
        evidence["fallback_diagnostics"].append({
            ...
            "message": "M006 S01 did not run live HTTP probes because required sandbox env was absent.",
            "missing_env": missing,
        })
    ...
    # Probe 1: extended route discovery
    p01 = probe_extended_route_discovery(client, company_id, plugin_key)
    evidence["probes"]["extended_route_discovery"] = p01

    # Probe 2: plugin install attempt
    p02 = probe_plugin_install_attempt(client, company_id, plugin_key, manifest)
    evidence["probes"]["plugin_install_attempt"] = p02

    # Probe 3: tool registry extended
    p03 = probe_tool_registry_extended(client, company_id, plugin_key)
    evidence["probes"]["tool_registry_extended"] = p03

    # Probe 4: plugin health deep
    p04 = probe_plugin_health_deep(client, company_id, plugin_key)
    evidence["runtime"] = {
        "version": p04.get("runtime_info", {}).get("version") or evidence["runtime"]["version"],
        "build": p04.get("runtime_info", {}).get("build") or evidence["runtime"]["build"],
    }
    evidence["probes"]["plugin_health_deep"] = p04
    ...
    if _is_passing(evidence):
        ...
        evidence["capability_promotions"] = [
            "runtime.extended_route_discovery",
            "runtime.plugin_install",
            "runtime.tool_registry_extended",
            "runtime.plugin_health_deep",
    ...
    evidence = _deep_redact(evidence)
    evidence["generated_at"] = started_at
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run bounded M006 S01 plugin live registration probes.")
    ...
    try:
        ...
        print(f"M006 S01 probe wrote blocker evidence (runner exception): {target}", file=sys.stderr)
        return 1
