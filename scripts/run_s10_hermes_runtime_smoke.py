#!/usr/bin/env python3
"""Run a bounded S10 Hermes runtime smoke through supported Paperclip HTTP surfaces.
...
S10 blocker artifact for validator/readiness closeout.
"""
...
DEFAULT_OUTPUT = Path("runtime-evidence/M002-S10-hermes-runtime-execution-proof.json")
S08_DECISION = Path("runtime-evidence/M002-S08-execution-path-decision-packet.json")
S08_REMEDIATION = Path("runtime-evidence/M002-S08-hermes-cli-environment-remediation.json")
...
def _read_config_defaults() -> dict[str, Any]:
    ...
    remediation = _load_json(S08_REMEDIATION)
    ...
    return {
        ...
        "remediation_ready": _nested_get(remediation, "conclusion.ready_for_bounded_t04_candidate"),
    }
...
def _base_evidence(args: argparse.Namespace, defaults: Mapping[str, Any], auth_meta: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "adapter": {"adapterType": ADAPTER_TYPE, "adapterConfig": defaults.get("adapter_config") or {}},
        "inputs": {
            ...
            "previous_evidence": {
                "decision_packet": str(S08_DECISION),
                "remediation": str(S08_REMEDIATION),
                ...
                "remediation_ready_for_bounded_smoke": defaults.get("remediation_ready"),
            },
        },
...
def _make_agent_body(args: argparse.Namespace, defaults: Mapping[str, Any]) -> dict[str, Any]:
    ...
    adapter_config.setdefault("provider", "openai-codex")
    adapter_config.setdefault("model", "gpt-5.3-codex")
...
def _blocker_codes(evidence: Mapping[str, Any]) -> list[str]:
    ...
    if test_environment:
        if test_environment.get("http_status") in (401, 403):
            ...
            codes.append("unsupported_codex_backend_config")
...
def run_smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    blocking_preflight = [
        ...
        in {
            ...
            "unsupported_codex_backend_config",
        }
    ...
    if isinstance(agent_id, str) and agent_id.strip():
        ...
        invoke_body = {
            "reason": "bos-light-m002-s10-hermes-runtime-smoke",
    ...
    if _is_passing_proof(evidence):
        ...
        evidence["capability_promotions"] = ["hermes.execution"]
    else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run S10 Hermes runtime smoke and write redacted proof/blocker evidence.")
    ...
    parser.add_argument("--agent-name", default="BOS Light S10 Hermes Codex Runtime Smoke", help="Name used for the supported Paperclip smoke agent.")
    parser.add_argument(
        "--agent-prompt",
        default=(
            "You are a bounded BOS Light Paperclip runtime smoke agent. Do not modify files, create approvals, "
            "access secrets, or call external project services except the configured LLM backend. Respond with only "
            "compact JSON shaped as {\"bos\":{\"schemaVersion\":\"s10-runtime-smoke/v1\",\"ok\":true,\"summary\":\"...\",\"sideEffects\":[]}}."
        ),
    )
    parser.add_argument(
        "--smoke-prompt",
        default=(
            "Return only compact JSON in resultJson.bos shape for BOS Light S10: "
            "{\"bos\":{\"schemaVersion\":\"s10-runtime-smoke/v1\",\"ok\":true,\"summary\":\"Hermes Codex backend smoke completed through Paperclip\",\"sideEffects\":[]}}. "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"S10 Hermes runtime smoke wrote {evidence.get('artifact_type')} evidence: {args.output}")
