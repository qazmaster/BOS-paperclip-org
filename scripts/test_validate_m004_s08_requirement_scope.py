...
class TestValidateM004S08RequirementScope(unittest.TestCase):
    ...
    def test_citation_allows_project_managed_gsd_symlink(self) -> None:
        """A .gsd citation may resolve through the project-managed artifact symlink."""
        self._set_root()
        project_store = self.tmpdir.parent / "s08-project-gsd-store"
        try:
            project_store.mkdir()
            citation_target = project_store / "milestones" / "M004-osbua3" / "M004-osbua3-VALIDATION.md"
            citation_target.parent.mkdir(parents=True, exist_ok=True)
            citation_target.write_text("stub GSD validation citation", encoding="utf-8")
            (self.tmpdir / ".gsd").symlink_to(project_store)

            data = _build_happy_ledger(str(Path("stub")))
            data["requirements"][0]["evidence_citations"][0]["path"] = ".gsd/milestones/M004-osbua3/M004-osbua3-VALIDATION.md"
            ledger_path = _write_fixture_ledger(self.tmpdir, data)

            errors = mod.ErrorCollector()
            mod.validate_ledger(ledger_path, errors)
            self.assertTrue(errors.ok, errors.errors)
        finally:
            import shutil
